...
export default function AuctionPage({ user }) {
  ...
  useEffect(() => {
    ...
    const handleNewBid = (data) => {
      ...
      if (data.highestBidder !== user?.userId) {
        toast(
          `${data.highestBidderName} bid $${data.currentHighestBid?.toLocaleString()}`,
          { icon: "🔥" },
        );
      }
    ...
    socket.on("leader-changed", (data) => {
      ...
      setServerInfo((prev) =>
        prev ? { ...prev, currentLeader: data.newLeader } : prev,
      );
    });
  ...
  const fetchAuction = async () => {
    try {
      ...
      const suggested =
        (res.data.auction.currentHighestBid || res.data.auction.startingPrice) +
        1;
  ...
  const handlePlaceBid = async (e) => {
    ...
    if (amount <= (auction?.currentHighestBid || 0)) {
      toast.error(
        `Bid must be higher than $${auction?.currentHighestBid?.toLocaleString()}`,
      );
      return;
    }
    try {
  ...
  if (loading)
    return (
      <div
        className="container"
        style={{ paddingTop: 60, display: "flex", justifyContent: "center" }}
      >
        <div
          style={{
            width: 36,
  ...
  const isOngoing =
    auction.status === "ONGOING" && new Date(auction.endTime) > new Date();
  ...
  const isMyTurnBlocked = lastBid?.userId === user?.userId;
  return (
    <div
      className="container"
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div
            ...
            style={{ padding: 28, animation: "fadeIn 0.4s ease" }}
          >
            <div
              style={{
                display: "flex",
            ...
            >
              <span
                className={`badge ${isOngoing ? "badge-ongoing" : "badge-ended"}`}
              >
                {isOngoing ? (
                  <>
                    <span
            ...
            >
              ...
              >
                ...
                >
                  {isOngoing
                    ? timeLeft
                    : format(new Date(auction.endTime), "MMM dd, HH:mm")}
                </div>
        ...
        <div style={{ position: "sticky", top: 80 }}>
          <div
            className="glass-card"
            style={{ padding: 24, animation: "fadeIn 0.5s ease" }}
          >
            <h2
              style={{
                fontSize: 17,
            ...
            {isOngoing ? (
              ...
              >
                ...
                <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                  {[10, 50, 100, 500].map((inc) => (
                    <button
                      ...
                      onClick={() =>
                        setBidAmount(
                          (auction.currentHighestBid + inc).toString(),
                        )
                      }
                    >
                      +${inc}
                ...
                <button
                  ...
                  disabled={bidLoading || !user || isMyTurnBlocked}
                ...
                {isMyTurnBlocked && (
                  ...
                  >
                    You cannot bid twice in a row. Wait for another user.
                  </p>
                )}
                {!user && (
                  <p
                ...
                >
                  ...
                  <br />
                  Bids are processed by the <strong>Leader</strong> with a
                  Lamport timestamp, then replicated to all followers instantly.
                </div>
              </form>
            ) : (
          ...
          >
            <div>
              <strong style={{ color: "var(--text-2)" }}>Starting:</strong> $
              {auction.startingPrice?.toLocaleString()}
            </div>
            <div>
              <strong style={{ color: "var(--text-2)" }}>Ends:</strong>{" "}
              {format(new Date(auction.endTime), "MMM dd,yyyy HH:mm")}
            </div>
            <div>
              <strong style={{ color: "var(--text-2)" }}>Bids:</strong>{" "}
              {bids.length} total
            </div>
            <div>
              <strong style={{ color: "var(--text-2)" }}>Rate limit:</strong> 5
              bids/second
            </div>
          </div>
        </div>
