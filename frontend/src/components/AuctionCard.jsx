...
export default function AuctionCard({ auction, onEnd, isOwner, isWatched, onToggleWatch }) {
  ...
  const timeLeft  = isLive ? formatDistanceToNow(endTime, { addSuffix: false }) : null;
  ...
  return (
    ...
    >
      {/* ── Image / Placeholder ─────────────────────────────── */}
      <div style={{
        height: 150, position: 'relative', flexShrink: 0,
        ...
        <div style={{ position: 'absolute', top: 10, left: 10 }}>
          <span className={`badge ${isLive ? 'badge-ongoing' : 'badge-ended'}`}>
            {isLive ? <><span className="live-dot" style={{ width: 5, height: 5 }} />LIVE</> : 'ENDED'}
          </span>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, color: 'rgba(240,244,255,0.5)', marginTop: 1 }}>
      ...
      <div style={{ padding: '13px 14px', display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
        ...
        <div style={{
          ...
          {timeLeft ? (
            ...
            <span>
              {auction.highestBidderName ? `Won by ${auction.highestBidderName}` : 'No bids'}
            </span>
          )}
          {isLive && auction.highestBidderName && (
            <span style={{ color: 'var(--text-3)', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 90, whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', gap: 7, marginTop: 'auto' }}>
          ...
          {isOwner && isLive && (
            <button
              ...
              style={{ color: 'var(--red)', borderColor: 'rgba(248,113,113,0.22)', flexShrink: 0 }}
            >
              End
            </button>
          )}
