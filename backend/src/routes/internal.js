...
const { handleElectionMessage, getLeaderState } = require('../distributed/leaderElection');
...
router.post('/replicate-bid', async (req, res) => {
  try {
    ...
    console.error(`[Internal][Server ${SERVER_ID}] Replication error:`, err.message);
...
router.post('/leader-election', async (req, res) => {
  try {
    ...
    console.error(`[Internal][Server ${SERVER_ID}] Election error:`, err.message);
    res.status(500).json({ error: err.message });
  }
});
